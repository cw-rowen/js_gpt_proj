/**
 * experiment.js — PsychoJS port for Pavlovia
 *
 * Key design decisions vs. the Python original:
 *  - info_type is assigned per-participant at runtime (not baked into conditions.csv)
 *  - conditions.csv contains only product rows (no info_type column)
 *  - All 4 info images per product are preloaded (we don't know the assignment ahead of time)
 *  - Two output files are replicated: Results_<ID>.csv and Timing_<ID>.csv
 *    Both are written via browser download at the end of the session
 *  - Peer names and participant ID come from the startup dialog
 *
 * FOLDER STRUCTURE (relative to experiment root on Pavlovia / GitLab):
 *
 *   experiment.js
 *   experiment.html
 *   conditions.csv
 *   stim/
 *     00_fixation/fixation.png
 *     01_product/airfryer.png  (keep spaces in filenames: "bath bomb.png" etc.)
 *     02_information/airfryer_01.png  airfryer_02.png  airfryer_03.png  airfryer_04.png ...
 *     03_question/credibility_EX.png  credibility_CON.png  credibility_PEER.png
 *                 credibility_general.png  preference.png
 *     04_intro/intro.png
 *
 * conditions.csv columns (no info_type — that's assigned at runtime):
 *   product_ENG, product_KOR, genre, classification, price_range
 */
...
const CFG = {
  ...
  scale_n        : 7,
  circle_radius  : 0.065,   // height units
  scale_y        : -0.15,   // norm
  numbers_y      : -0.27,
  desc_y         : -0.35,
  label_x        : -0.75,
...
// Expert labels — mirrors expert_labels.csv
// Keys are lowercased + trimmed product_ENG values
...
function getExpertLabel(productENG) {
  return EXPERT_LABEL_MAP[productENG.trim().toLowerCase()] || null;
}
...
async function runExperiment() {
  ...
  for (let tIdx = 0; tIdx < nTrials; tIdx++) {
    ...
    // Filenames — keep spaces exactly as Python does
    const productFile = `${trial.product_ENG}.png`;
    const infoFile    = `${trial.product_ENG}_${code}.png`;
...
// ─────────────────────────────────────────────────────────────────────────────
const PRODUCTS = [
  'airfryer','bath bomb','body lotion','body mist','bread','cable organizer',
  'cake','canned tuna','caviar','chips','chocolate','cookies','cup ramen',
  'decorative magnet','decorative tape','disposable camera','egronomics chair seat',
  'egyptian cotton sheets set','electric toothbrush','fountain pen','frozen pizza',
  'gaming console','hair brush','instant coffee','instant coffee maker','laptop stand',
  'leather notebook','LED mood light','lip balm','luxury perfume','mango set',
  'manuka honey','massaging roller','memo pads','mini camera','mouse pad','multitab',
  'oil pastels','organic olive oil','paper weight','pen','portable charger',
  'portable humidifier','projector','rc car','roll-on mini perfume','scented hand cream',
  'scientific calculator','shampoo','singing bowl','skin care device','SSD external storage',
  'stamp','steak 500g','stickers','supplements','tablet','tea gift set',
  'vacuum cleaner','VR headset','wax burner','wax seal kit','weighted blanket',
  'wireless keyboard',
];

const resources = [
  { name:'conditions.csv',          path:'conditions.csv' },
  { name:'fixation.png',            path:'stim/00_fixation/fixation.png' },
  { name:'intro.png',               path:'stim/04_intro/intro.png' },
  { name:'credibility_EX.png',      path:'stim/03_question/credibility_EX.png' },
  { name:'credibility_CON.png',     path:'stim/03_question/credibility_CON.png' },
  { name:'credibility_PEER.png',    path:'stim/03_question/credibility_PEER.png' },
  { name:'credibility_general.png', path:'stim/03_question/credibility_general.png' },
  { name:'preference.png',          path:'stim/03_question/preference.png' },
  ...PRODUCTS.map(p => ({ name:`${p}.png`, path:`stim/01_product/${p}.png` })),
  ...PRODUCTS.flatMap(p =>
    ['01','02','03','04'].map(c => ({ name:`${p}_${c}.png`, path:`stim/02_information/${p}_${c}.png` }))
  ),
];
